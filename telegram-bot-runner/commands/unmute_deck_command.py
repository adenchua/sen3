...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            # get only unmuted decks
            decks_response = await client.get(GET_DECKS_URL, params={"isActive": "0"})
            ...
            if len(decks) == 0:
                await update.message.reply_text(
                    "There are no muted decks", reply_markup=ReplyKeyboardRemove()
                )
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            await query.edit_message_text(
                "Deck unmuted! You will now receive notifications from this deck"
            )
        except httpx.HTTPStatusError as http_error:
            logger.error(f"http error: {http_error}")
        except Exception as error:
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text(
        "Operation cancelled. No decks unmuted", reply_markup=ReplyKeyboardRemove()
    )
    context.user_data.clear()
    return ConversationHandler.END
...
unmute_deck_conv_handler = ConversationHandler(
    entry_points=[CommandHandler("unmutedeck", start)],
